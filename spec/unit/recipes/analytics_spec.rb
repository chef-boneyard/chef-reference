...
describe 'chef-reference::analytics' do
  ...
  context 'running the recipe with wildcard cert data' do
    ...
    it 'creates the certificate directory' do
      expect(chef_run).to create_directory('/var/opt/opscode-analytics/ssl/ca').with(recursive: true)
    end

    it 'creates the pem file' do
      expect(chef_run).to create_file('/var/opt/opscode-analytics/ssl/ca/analytics.chefspec.example.com.pem').with(
        content: 'pem file'
      )
    end
  end
